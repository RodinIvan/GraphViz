...
public class MainClass{
	...
	public static void main(String[] args) {
		...
		try{	
			...
			for(int i=2; i<=26; i++){				//в thread RUN
				...
				/*
				Viewer viewer = new Viewer(null, Viewer.ThreadingModel.GRAPH_IN_ANOTHER_THREAD); //Это все из цикла
...
class GraphConstructor {
	...
	public void fillGraph(Viewer viewer){
		...
		for (Node node : graph) {
			//System.out.println(node.getId());
			//System.out.println(node.getEdgeSet());
		    if(node.getId().equals("крупная_компания"))
		    {
		    	graph.removeNode(node); //Пофиксить, чтобы удалял не только из графа но и из графинфо
		    	graphInfo.deleteNode(node.getId().toString().toLowerCase());
		    }
